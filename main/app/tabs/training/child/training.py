...
def training_model_tab():
    with gr.Blocks() as tab:
        ...
        gr.Markdown(translations["training_markdown"], elem_classes=["markdown-header"])

        # Main Layout: Split into Preprocessing, Feature Extraction, and Training/Indexing
        with gr.Row():
            # Preprocessing Section
            with gr.Column(scale=1):
                gr.Markdown("### Preprocessing")
                with gr.Group():
                    training_name = gr.Textbox(label=translations["modelname"], info=translations["training_model_name"], placeholder=translations["modelname"], interactive=True)
                    ...
                    preprocess_info = gr.Textbox(label=translations["preprocess_info"], interactive=False)
            ...
            with gr.Column(scale=1):
                ...
                with gr.Group():
                    ...
                    with gr.Accordion(label=translations["hubert_model"], open=False):
                        ...
                        extract_embedders_custom = gr.Textbox(label=translations["modelname"], info=translations["modelname_info"], placeholder="hubert_base", interactive=True, visible=extract_embedders.value == "custom")
                    
                    extract_button = gr.Button(translations["extract_button"], variant="primary")
                    extract_info = gr.Textbox(label=translations["extract_info"], interactive=False)

            # Training and Indexing Section
            with gr.Column(scale=1):
                gr.Markdown("### Training & Indexing")
                with gr.Group():
                    with gr.Row():
                    ...
                    with gr.Accordion(label=translations["setting"], open=False):
                        ...
                        model_author = gr.Textbox(label=translations["training_author"], info=translations["training_author_info"], placeholder=translations["training_author"], interactive=True)
                        ...
                        with gr.Accordion(translations["custom_pretrain_info"], open=False, visible=custom_pretrain.value and not not_use_pretrain.value) as pretrain_setting:
                            pretrained_D = gr.Dropdown(label=translations["pretrain_file"].format(dg="D"), choices=pretrainedD, value=pretrainedD[0] if pretrainedD else '', interactive=True, allow_custom_value=True)
                            pretrained_G = gr.Dropdown(label=translations["pretrain_file"].format(dg="G"), choices=pretrainedG, value=pretrainedG[0] if pretrainedG else '', interactive=True, allow_custom_value=True)
                            refesh_pretrain = gr.Button(translations["refesh"])
                    
                    training_info = gr.Textbox(label=translations["train_info"], interactive=False)

                    # Model Export Section
                    with gr.Accordion(translations["export_model"], open=False):
                        model_file = gr.Dropdown(label=translations["model_name"], choices=model_name, value=model_name[0] if model_name else "", interactive=True, allow_custom_value=True)
                        index_file = gr.Dropdown(label=translations["index_path"], choices=index_path, value=index_path[0] if index_path else "", interactive=True, allow_custom_value=True)
                        with gr.Row():
                            refesh_file = gr.Button(translations["refesh"])
        ...
        preprocess_button.click(
            fn=preprocess,
            inputs=[
                training_name, training_sr, cpu_core, preprocess_cut, process_effects,
                dataset_path, clean_dataset, clean_dataset_strength
            ],
        ...
        training_button.click(
            ...
            inputs=[
                ...
                deterministic, benchmark, optimizer
            ],
            outputs=[training_info],
