...
class Separator:
    def __init__(self, logger, model_file_dir=config.configs["uvr5_path"], output_dir=None, output_format="wav", output_bitrate=None, normalization_threshold=0.9, output_single_stem=None, invert_using_spec=False, sample_rate=44100, mdx_params={"hop_length": 1024, "segment_size": 256, "overlap": 0.25, "batch_size": 1, "enable_denoise": False}):
        self.logger = logger
        ...
        if normalization_threshold <= 0 or normalization_threshold > 1:
            raise ValueError(translations[">0or=1"])

        self.output_single_stem = output_single_stem
        if output_single_stem is not None:
            self.logger.debug(translations["output_single"].format(output_single_stem=output_single_stem))

        self.invert_using_spec = invert_using_spec
        if self.invert_using_spec:
            self.logger.debug(translations["step2"])

        self.sample_rate = int(sample_rate)
        self.arch_specific_params = {"MDX": mdx_params}
    ...
    def list_supported_model_files(self):
        response = requests.get(codecs.decode(f"uggcf://uhttvatsnpr.pb/{user_hf}/enj/znva/wfba/hie_zbqryf.wfba", "rot_13"))
        response.raise_for_status()
        model_downloads_list = response.json()
        self.logger.debug(translations["load_download_json"])
        return {"MDX": {**model_downloads_list["mdx_download_list"], **model_downloads_list["mdx_download_vip_list"]}}
    ...
    def download_model_files(self, model_filename):
        ...
        for model_type, model_list in supported_model_files_grouped.items():
            for model_friendly_name, model_download_list in model_list.items():
                ...
                model_repo_url_prefix = codecs.decode(f"uggcf://uhttvatsnpr.pb/{user_hf}/erfbyir/znva/hie5_zbqryf", "rot_13")
    ...
    def separate(self, audio_file_path):
        ...
        self.print_uvr_vip_message()
        self.logger.debug(translations["separator_success"])
