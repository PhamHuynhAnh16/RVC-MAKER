...
from main.library.utils import load_embedders_model, check_embedders
...
class AutoPitch:
    def __init__(self, vc, rvc_npz_path, emb_npz_path, pitch_guidance = True, version = "v1", energy_use = False, device = "cpu"):
        ...
        check_embedders(self.hubert_base, self.hubert_mode)
        self.vc = vc
    ...
    def autopitch(self, model, net_g, sid, index = None, big_npy = None, tgt_sr = 32000):
        ...
        # no need to add this config
        if sim_male > sim_female: return round(1127 * np.log(1 + sim_male * 100 / 700), 1), True #if configs.get("calc_pitch", False) else 155.0, True
        elif sim_male < sim_female: return round(1127 * np.log(1 + sim_female * 10000 / 700) / 10, 1), True #if configs.get("calc_pitch", False) else 255.0, True
